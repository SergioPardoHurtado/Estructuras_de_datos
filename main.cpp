...
void contienetes(list<Continentes> &listaContinentes) {
  ...
  for (auto &continente : listaContinentes) {
    ...
    if (continente.continente == "África") {
    ...
    if (continente.continente == "Asia") {
      agregarPaisAContinente(continente, "Afganistán");
...
void Inicializar(list<jugador> &listaJugadores,
  ...
  int jp;
  string nombre;
  cout << "Cuántos jugadores van a jugar (mínimo 3, máximo 6):" << endl;
  cin >> jp;

  int totalFichas = 40 - (jp - 3) * 5; // Calcular el total de fichas según el número de jugadores
  ...
  for (int i = 0; i < jp; i++) {
    ...
    do {
      cout << "Elija un país para " << nuevoJugador.nombre << ": ";
      cin >> paisElegido;
      ...
      if (it != paisesDisponibles.end() && it->second) {
        ...
        cout << "País no válido o ya seleccionado. Intente nuevamente." << endl;
        paisValido = false;    
      } 
    } while (!paisValido);
    ...
    int fichasColocadas;
    do {
        cout << "Ingrese la cantidad de fichas que colocará en " << paisElegido << ": ";
        cin >> fichasColocadas;
        ...
        if (fichasColocadas <= nuevoJugador.fichas) {
            ...
            cout << "No tiene suficientes fichas. Ingrese un número válido." << endl;
        }
    } while (!paisValido);
...
void Turno_IDJugador(list<Continentes> &listaContinentes, list<jugador> &listaJugadores, bool inicializar,
  ...
  if (!jugadorEncontrado) {
    cout << "El jugador con ID " << iden << " no existe. Intente nuevamente." << endl;
  ...
  if (inicializar == true) {
    if (juegoterminado == false) {
      ...
      string territorioOrigen, territorioDestino;
        ...
        for (jugador &jpr : listaJugadores) {
          if (jpr.id != jugadorSeleccionado.id) {
            // Saltar a la siguiente iteración si el jugador no es el seleccionado
            continue;
          }
          ...
          if (jpr.fichas > 0) {
           string paisElegido;
            ...
            do {
              cout << "Elija un país para colocar " << jpr.fichas << " tropa(s): ";
              cin >> paisElegido;
              ...
              if (!paisValido) {
                cout << "País no válido o no controlado por el jugador. Intente nuevamente." << endl;
              }
           ...
           do {
            ...
            cout << "(Turno del jugador " << jpr.nombre << ") Elige el territorio de origen para el ataque: ";
            ...
            if (!territorioValido) {
                cout << "No tienes control sobre el territorio " << territorioOrigen << ". Intenta nuevamente." << endl;
            }
          ...
          do {
            ...
            cout << "(Turno del jugador " << jpr.nombre << ") Elige el territorio de destino para el ataque: ";
            cin >> territorioDestino;

            
            for (jugador &opponent : listaJugadores) {
                if (opponent.nombre != jpr.nombre) { // No se busca en los propios territorios
            ...
            if (!DestinoValido) {
                cout << "El territorio " << territorioDestino << " no es controlado por otro jugador. Intenta nuevamente." << endl;
            }
    ...
    int unidadesGanadasAtacante = 0; // Inicializa estas variables según tu lógica
    int unidadesGanadasDefensor = 0;

    if (dadoJugador1 < dadoJugador2) {
        for (Paises &pais : jpr.paises_domados) {
            if (pais.pais == territorioOrigen) {
                unidadesGanadasAtacante = pais.tropas; // Resta las tropas del país de origen al atacante
                jpr.fichas -= pais.tropas;
                pais.tropas = 0; // El país de origen pierde todas las tropas
                break;
            }
        }
    }
    ...
    std::cout << "Unidades ganadas por el atacante: " << unidadesGanadasAtacante << std::endl;
    std::cout << "Unidades ganadas por el defensor: " << unidadesGanadasDefensor << std::endl;
}

       cout << "(Turno terminado correctamente) El turno del jugador " << jpr.nombre << " ha terminado." << endl;
        }
    ...
    if (comando == "inicializar") {
      ...
      if (inicializar != true) {
        if (cargarc == true) {
          ...
          cout << "\n(Archivo vacío o incompleto, seccion aun sin implementar) "
                  "el archivo no contendria la información válida para "
        ...
        if (juegoterminado != true) {
          if (comandoT == true) {
            cout << "\n(Comando correcto, seccion aun sin implementar) Para "
                    ...
                    "<territorio_2>, <territorio_3>, ..., <territorio_m>. Debe "
      ...
      if (inicializar == true) {
        if (juegoterminado != true) {
          if (conquistV != true) {
            cout << "\n(Jugador no válido, seccion aun sin implementar) La "
                    "conquista más barata es avanzar "
      ...
      cout << "\n(Inicialización satisfactoria) El juego se ha inicializado "
              "correctamente.\n";
      cout << "\nDescipcion del conmando:\n";
      cout << "\nRealiza las operaciones necesarias para inicializar el juego, "
              "de acuerdo a las instrucciones entregadas. De esta forma, el "
              "comando debería preguntar la cantidad de jugadores, para cada "
              "jugador su nombre o identificador, y luego, por turnos, "
              "preguntar a cada jugador en qué territorio desea ubicar sus "
              "unidades de ejército. En cada turno, el jugador sólo puede "
              "indicar un único territorio a ocupar. En este comando es clave "
              "utilizar una interfaz adecuada de asignación de territorios que "
              "sea fácil de seguir para los jugadores en pantalla.\n";
      ...
      cout << "\n(Juego no inicializado) Esta partida no ha sido inicializada "
              "correctamente.\n";
      cout << "\n(Juego terminado) Esta partida ya tuvo un ganador.\n";
      cout << "\n(Jugador no válido) El jugador <id_jugador>no forma parte de "
              "esta partida.\n";
      ...
      cout << "\nRealiza las operaciones descritas dentro del turno de un "
              ...
              "forma, el comando debería primero informar al jugador cuántas "
              "unidades adicionales puede reclamar, para luego preguntarle en "
              "cuáles de sus territorios las quiere asignar y en qué "
              "cantidad.A continuación, debería preguntar la configuración del "
              "ataque, desde cuál territorio y hacia cuál "
              "territorio,verificando las condiciones ya descritas. Luego "
              "debería informar los valores obtenidos con los dados, y "
              "lacantidad de unidades que se ganan o pierden. Este proceso se "
              "repite hasta que alguno de los dos territoriosse quede sin "
              "unidades, o hasta que el atacante decida detenerse. Finalmente, "
              "el comando debería preguntar al jugador los territorios vecinos "
              "que desea seleccionar para la fortificación, así como la "
              "cantidad de unidadesque se trasladarán de uno al otro. De "
              "nuevo, en este comando es clave utilizar una interfaz adecuada "
              "que sea fácil de seguir para los jugadores en pantalla.\n";
    }
        ...
        cout << "\nEl estado actual del juego es guardado en un archivo de "
                ...
                "jugador, la cantidadde países que ocupa, el identificador de "
                "cada país junto con la cantidad de unidades de ejército en "
                "él, lacantidad de tarjetas que posee y el identificador de "
                "cada tarjeta. Note que este comando guarda un archivode texto "
                "plano, sin codificación.\n";
      }
        ...
        cout << "\nEl estado actual del juego es guardado en un archivo "
                "binario (con extensión .bin) con lainformación (la misma que "
                "se almacenaría en un archivo de texto normal, ver comando "
                "guardar) comprimida,utilizando la codificación de Huffman en "
        ...
        cout << "\nPosibles salidas:\n";
        cout << "\n(Juego en curso) El juego ya ha sido inicializado.\n";
        cout << "\n(Archivo vacío o incompleto) el archivo no contiene "
                "información válida para inicializar el juego.\n";
        cout << "\nDescipcion del conmando:\n";
        cout << "\nInicializa el juego con los datos contenidos en el archivo "
                "identificado por <nombre_archivo>.El archivo debería contener "
                "la información descrita en el comando guardar. El comando "
        ...
        cout
            << "\nEl programa debe calcular el costo y la secuencia de "
               ...
               "atacar debe ser aquel que el jugadortenga controlado más cerca "
               "al dado por el jugador. Esta información se analiza desde el "
        ...
        cout << "\n(Juego no inicializado) Esta partida no ha sido "
                "inicializada correctamente.\n";
        cout << "\n(Juego terminado) Esta partida ya tuvo un ganador.\n";
        cout << "\n(Jugador no válido) La conquista más barata es avanzar "
        ...
        cout << "\nDe todos los territorios posibles, calcular aquel que pueda "
                "implicar un menor número de unidades de ejército perdidas. "
                "Esta información se analiza desde el punto de vista del "
      ...
      cout << "Para revisar las opciones de comandos disponibles teclee el "
